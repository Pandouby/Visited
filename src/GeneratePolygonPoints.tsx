...
function geoPolygonTriangulate(
  polygon,
  {
  ...
  const crossesPoleOrAntimeridian =
    minLng > maxLng || // crosses antimeridian
  ...
  if (crossesPoleOrAntimeridian) {
    ...
    const pntMap = new Map(
      points.map(([lng, lat], idx) => [`${lng}-${lat}`, idx])
    );
    ...
    for (let i = 0, len = delaunay.triangles.length; i < len; i += 3) {
      ...
      // exclude edge triangles outside polygon perimeter or through holes
      if (inds.some((ind) => ind < edgePoints.length)) {
        const triangleCentroid = [0, 1].map((coordIdx) =>
          mean(triangle, (p) => p[coordIdx])
        );
        if (
          !pointInside(
            triangleCentroid,
  ...
  // calc uvs
  const lngUvScale = scaleLinear(
    extent(points, (d) => d[0]),
    [0, 1]
  );
  const latUvScale = scaleLinear(
    extent(points, (d) => d[1]),
    [0, 1]
  );
...
function getInnerGeoPoints(polygon, maxDistance) {
  ...
  if (
    Math.min(Math.abs(maxLng - minLng), Math.abs(maxLat - minLat)) < maxDistance
  )
    return [];

  const crossesPoleOrAntimeridian =
    minLng > maxLng || maxLat >= 89 || minLat <= -89;
  ...
  return getGeoSpiralGrid(maxDistance, {
    ...
    pointInside(pnt, boundariesGeojson, crossesPoleOrAntimeridian)
  );
}
...
function getGeoSpiralGrid(
  ...
  const getPntLat = (idx) =>
    (Math.acos((2 * idx) / numPoints - 1) / Math.PI) * 180 - 90;
  const getPntIdx = (lat) =>
    (numPoints * (Math.cos(((lat + 90) * Math.PI) / 180) + 1)) / 2;
